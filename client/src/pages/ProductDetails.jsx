...
function ProductDetails() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        ...
        const otherProducts = filteredProducts.filter((item) => {
          const itemCategoryId = item?.category?._id || item?.category;
          return itemCategoryId !== currentCategoryId;
        });

        const finalProducts = [...sameCategoryProducts, ...otherProducts];

        setYouMayLikeProducts(finalProducts.slice(0, 5));

        setRelatedProducts(
          finalProducts.length > 5
            ? finalProducts.slice(5, 11)
  ...
  return (
    <div className="min-h-screen bg-[#f7fafc]">
      ...
      <main className="mx-auto max-w-[1180px] px-0 py-0 sm:px-4 sm:py-5">
        <div className="hidden sm:block">
          <Breadcrumb
            categoryId={product?.category?._id}
            categoryName={product?.category?.name}
        ...
        {error && (
          <div className="rounded-md border bg-white p-6 text-center text-red-500">
            {error}
          </div>
        )}
